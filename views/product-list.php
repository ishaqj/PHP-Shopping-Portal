...
if(!empty($showAllProducts)) : ?>
    ...
    <?php foreach($showAllProducts as $k => $product) : ?>
        ...
        <?= $products->setId($product->id);?>
            <div class="col-xs-6 col-sm-6 col-md-3">
                ...
                <?php endif;?>
                <div id="product">
                    <div class="photo"><img src="<?=$products->fetchImage()->image?>" class="img-responsive"  alt="<?=$product->name?>"></div>
    ...
    <ul class="pagination">
        ...
        <?php if($page<$total_pages) :?>
            <?php if(isset($category_id) && !empty($category_id)) : ?>
                <li><a href='<?=$page_url?>?id=<?=$category_id?>&page=<?=$total_pages?>' title='Last page is <?=$total_pages?>.'>Sista sidan</a></li>
            <?php else: ?>
                <li><a href='<?=$page_url?>?page=<?=$total_pages?>' title='sista sidan är <?=$total_pages?>.'>Sista sidan</a></li>
            <?php endif; ?>
        <?php endif; ?>
    </ul>
    </div>
